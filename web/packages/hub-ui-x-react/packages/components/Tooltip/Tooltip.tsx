...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  const [tooltipPosition, setTooltipPosition] = useState({
    left: 0,
    top: 0,
    placement: placement as NonNullable<TooltipProps['placement']>,
  });
  ...
  const updatePosition = useCallback(() => {
    if (triggerRef.current && tooltipRef.current) {
      ...
      const adjusted = adjustPosition(newPosition);
      setTooltipPosition({
        left: adjusted.left,
        top: adjusted.top,
        placement: adjusted.placement || placement,
      });
    }
  }, [placement]);
  ...
  useLayoutEffect(() => {
    if (isVisible) {
      // useLayoutEffect 在浏览器绘制前同步执行,首次显示时直接测量并写入正确坐标,
      // 避免先以 (0,0) 渲染再跳位的"左上角闪一下"现象。
      updatePosition();
