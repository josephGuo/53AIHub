...
export function ChunkConfig({
  ...
  useEffect(() => {
    ...
    // 解析 identifier_level/mode 到 internalState
    const parseRule = (prefix: "knowledge" | "index") => {
      ...
      // 空字符串表示"整篇"（历史兼容，现在应该用 mode: 'whole'）
      if (rule === "" || mode === "whole") {
        setInternalState((prev) => ({
          ...prev,
          [`${prefix}_chunking_type`]: CHUNK_TYPE.WHOLE,
        }));
        return;
      }

      // undefined/null 表示使用默认值，不做任何设置
      if (!rule) return;
  ...
  // Watch internalState changes - 拆分为独立的同步，避免不必要触发
  useEffect(() => {
    syncToConfig("knowledge");
    // 仅监听 knowledge 相关字段变化
  ...
  const handleBlurMaxLength = (type: "knowledge" | "index") => {
    ...
    // Check if the current type is WHOLE, if so, enforce max_length to 5000
    const chunkingType =
      type === "knowledge"
        ? internalState.knowledge_chunking_type
        : internalState.index_chunking_type;
              ...
              <div className="flex items-center">
                <Radio.Group
                  ...
                  onChange={(e) => {
                    ...
                    } else if (newType === CHUNK_TYPE.WHOLE) {
                      setInternalState((prev) => ({
                        ...prev,
                        knowledge_chunking_type: CHUNK_TYPE.WHOLE,
                      }));
                      updateConfig({
                        parent_chunk: {
                          ...config.parent_chunk!,
                ...
                <Radio.Group
                  ...
                  onChange={(e) => {
                    ...
                    if (newType === CHUNK_TYPE.DEFAULT) {
                      ...
                      setInternalState((prev) => ({
                        ...prev,
                        index_chunking_type: CHUNK_TYPE.WHOLE,
                      }));
                      updateConfig({
                        child_chunk: {
                          ...config.child_chunk!,
                    ...
                    <div className="flex items-center">
                      ...
                      <InputNumber
                        ...
                        disabled={
                          internalState.index_chunking_type ===
                            CHUNK_TYPE.DEFAULT ||
                          internalState.index_chunking_type === CHUNK_TYPE.WHOLE
                        }
