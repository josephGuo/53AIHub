...
export const MineAudioView = forwardRef<MineAudioViewRef, MineAudioViewProps>(
  function MineAudioView({ keyword = "", onPreview, refreshKey = 0, fileRefreshKey, dirRefreshKey, enableFavorite = true }, ref) {
    ...
    const [moveToSourceItem, setMoveToSourceItem] = useState<AudioItem | null>(null);
    ...
    // 「移动到」弹窗适配：透传 path/keyword/type 到 recordingApi.getRecordings
    ...
    const handleMoveToConfirm = useCallback(async (targetPath: string) => {
      ...
      try {
        const fullPath = moveToSourceItem.rawData.path || ""
        const doubleExtMatch = fullPath.match(AUDIO_DOUBLE_EXT_REGEX)
        let ext: string
        if (doubleExtMatch) {
          ext = doubleExtMatch[0]
        ...
        const fileExtNoDot = ext.replace(/^\./, "")
        const newPath = buildNewPath(
          { name: moveToSourceItem.name, isfolder: moveToSourceItem.isfolder, file_ext: fileExtNoDot },
          targetPath
        )
        await filesApi.rename({ id: moveToSourceItem.id, path: newPath })
        message.success(t("mine.moved"))
        loadFiles(currentPath, true, !moveToSourceItem.isfolder, moveToSourceItem.isfolder)
    ...
    }, [moveToSourceItem, currentPath, loadFiles, t])
      ...
      <div className="h-[calc(100%-32px)] flex flex-col">
        ...
        <MoveToModal
          ...
          sourceItem={moveToSourceItem ? {
            ...
            path: moveToSourceItem.rawData.path,
            isfolder: moveToSourceItem.isfolder,
            file_ext: moveToSourceItem.file_ext,
            icon: moveToSourceItem.icon,
          } : null}
          title={t("mine.recording")}
          fetchDirs={moveToFetchDirs}
          onConfirm={handleMoveToConfirm}
          onCancel={() => { setMoveToModalVisible(false); setMoveToSourceItem(null) }}
          resolveFileIcon={() => getPublicPath("/images/file/recrod.png")}
        />
      </div>
    );
  },
