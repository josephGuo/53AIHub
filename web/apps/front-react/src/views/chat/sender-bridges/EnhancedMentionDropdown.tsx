 ...
 *
 * 4 个入口:
 *   1. @ 从知识库里选择  → onOpenLibrary(ChatContainer 渲染 SpaceDialog)
 ...
 *
 * 入口的可见性:
 *   - 入口 1 由 hasKnowledgeBase 控制;入口 2、3 由 workbench 控制;入口 4 由 recording 控制。
...
export function EnhancedMentionDropdown(props: EnhancedMentionDropdownProps) {
  ...
  // 入口可见性(对齐 legacy:hasKnowledgeBase / WORKBENCH / RECORDING)
  const showKnowledgeEntry = Boolean(hasKnowledgeBase) && checkVersion(VERSION_MODULE.KNOWLEDGE_BASE);
  const showUploadsEntry = checkVersion(VERSION_MODULE.WORKBENCH);
  const showAIGeneratedEntry = checkVersion(VERSION_MODULE.WORKBENCH);
  const showRecordingsEntry = checkVersion(VERSION_MODULE.RECORDING);
  ...
  const handleEntryClick = (entry: () => void) => {
    // 关闭下拉,然后触发外部 dialog
    onClose?.();
    entry();
  };
  ...
  return (
    ...
    >
      ...
      {/* 列表 */}
      <div className="enhanced-mention-dropdown__list">
        {hasKnowledgeBase && (
          <>
            <div className="enhanced-mention-dropdown__section-title">最近访问</div>
            <div className="enhanced-mention-dropdown__items">
              ...
              {!searchLoading && displayList.length === 0 && (
                <div className="enhanced-mention-dropdown__empty">
                  {searchKeyword.trim() ? "没有找到相关文件" : "没有匹配项，试试更换关键词"}
                </div>
              )}
            </div>
          </>
