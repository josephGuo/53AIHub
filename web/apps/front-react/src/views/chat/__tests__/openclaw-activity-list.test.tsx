...
describe("OpenClawActivityList", () => {
  ...
  it("renders tool.result output without repeating the command as tool input", () => {
    const activity = buildOpenClawActivity({
      kind: "tool.result",
      payload: {
        data: {
          phase: "result",
          name: "exec",
          toolCallId: "chatcmpl-tool-pdfinfo",
          args: { command: 'pdfinfo "/tmp/input.pdf" 2>&1' },
          result: {
    ...
    expect(activity?.title).toBe("Tool output");
    expect(activity?.tool?.input).toBe("");
    expect(activity?.tool?.output).toContain("Pages:           11");

    render(<OpenClawActivityList items={activity ? [activity] : []} />);

    expect(screen.queryByText("TOOL INPUT")).not.toBeInTheDocument();
    expect(screen.getByText("TOOL OUTPUT")).toBeInTheDocument();
    expect(screen.getByText(/Pages:\s+11/)).toBeInTheDocument();
  });

  it("reads QClaw aggregated tool result details as output", () => {
    const activity = buildOpenClawActivity({
      kind: "tool.result",
      payload: {
        data: {
    ...
    expect(activity?.tool?.input).toBe("");
    expect(activity?.tool?.output).toContain("Title:           Untitled");
  });
