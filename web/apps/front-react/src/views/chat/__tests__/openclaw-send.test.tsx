...
// unify-chat-adapters：useChatSend 通过 useChatAdapters() 读 conversationApi，
// 必须在 ChatConfigProvider 上下文内调用。
function wrapWithChatAdapters(conversationApi: IConversationApi) {
  // workflowApi/agentApi 是 useChatSend 不读的最小占位实现。
  return ({ children }: { children: ReactNode }) => (
    <ChatConfigProvider
      adapters={{
        conversationApi,
        agentApi: { detail: vi.fn(), list: vi.fn(), myDetail: vi.fn(), myList: vi.fn() } as any,
...
describe("OpenClaw send flow", () => {
  ...
  it("does not let a stale loading active OpenClaw snapshot reopen a completed hydrated row", () => {
    const sessionId = "agent:main:dashboard:test";
    const next = mergeOpenClawActiveMessageIntoList(
      [
        {
          ...
          conversation_id: sessionId,
          openclawTurn: {
            turnKey: `${sessionId}:turn:client-books`,
            sessionId,
            status: "completed",
            maxSeq: 360,
            events: [
              {
                eventId: "history-books-answer",
                sessionId,
                seq: 355,
                kind: "assistant.message",
                payload: { content: "我已经成功从网上搜索并整理了10本书的总结。" },
                source: "history",
              },
              {
                eventId: "history-books-completed",
                sessionId,
                seq: 360,
                kind: "run.completed",
                payload: {},
      ...
      {
        id: "active-books",
        _openclawClientMessageId: "client-books",
        _openclawTurnStartSeq: 340,
        question: "从网上搜索10本书并总结",
        answer: "",
        loading: true,
        conversation_id: sessionId,
        openclawTurn: {
          ...
          events: [
            {
              eventId: "stale-thinking",
              sessionId,
              seq: 350,
              kind: "assistant.thinking",
              payload: { content: "The user asked for ten books." },
              source: "stream",
            },
          ],
        },
        openclawTimelineItems: [
          {
            ...
            seq: 350,
            sessionId,
            content: "The user asked for ten books.",
          },
        ],
      } as any,
      sessionId
    ...
    expect(next).toHaveLength(1);
    expect(next[0].id).toBe("history-books");
    expect(next[0].loading).toBe(false);
    expect(next[0].answer).toBe("我已经成功从网上搜索并整理了10本书的总结。");
    expect(next[0].openclawTimelineItems!.filter((item: any) => item.type === "answer")).toHaveLength(1);
  });
  ...
  it("sends completions when a message list baseline is provided", async () => {
    ...
    const { result } = renderHook(() => useChatSend(), { wrapper: wrapWithChatAdapters(conversationApi as any) });
  ...
  it("does not send to OpenClaw when selected skill ensure fails", async () => {
    const completions = vi.fn().mockResolvedValue(undefined);
    const conversationApi = {
      create: vi.fn(),
      list: vi.fn(),
      ...
      del: vi.fn(),
      events: vi.fn().mockResolvedValue({ data: { events: [] } }),
      ensureSkill: vi.fn().mockResolvedValue({
        data: {
          data: {
            ok: false,
            status: "failed",
    ...
    expect(String((thrown as Error).message)).toContain("skill install failed");
    expect(conversationApi.ensureSkill).toHaveBeenCalledTimes(1);
    expect(completions).not.toHaveBeenCalled();
    expect(onMessageListChange).not.toHaveBeenCalled();
  });

  it("sends selected OpenClaw skill and uploaded files in the same request", async () => {
    const completions = vi.fn().mockResolvedValue(undefined);
    const ensureSkill = vi.fn().mockResolvedValue({
      data: {
        data: {
          ok: true,
          status: "up_to_date",
        ...
        files: [
          {
            ...
            preview_key: "probe-preview",
            preview_url: "http://localhost:9001/api/preview/probe-preview",
            signed_download_url: "http://localhost:9001/api/preview/probe-preview",
          },
        ],
        skill: {
          skill_id: "skill-1",
    ...
    expect(completions.mock.calls[0][0].messages.at(-1).content).toBe("测试技能效果");
    expect(completions.mock.calls[0][0].messages.at(-1).content).not.toContain("/openclaw_pdf_probe");
    expect(completions.mock.calls[0][0].messages.at(-1).content).not.toContain('"file_name":"probe.txt"');
    expect(renderedMessages.at(-1).uploaded_files).toEqual([
      expect.objectContaining({
        id: "file-1",
        file_name: "probe.txt",
      }),
    ]);
    expect(renderedMessages.at(-1).question).toBe("测试技能效果");
    expect(renderedMessages.at(-1).skill).toEqual(
      expect.objectContaining({
        skill_name: "openclaw_pdf_probe",
      })
    );
